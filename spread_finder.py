...
try:
    ...
    st.warning("⚠️ Google API Key not found. AI Narrative will be disabled.")
...
def newton_vol_call(S, K, T, C, r, sigma):
    """Calculates Implied Volatility for a Call using Newton-Raphson."""
...
def newton_vol_put(S, K, T, P, r, sigma):
    """Calculates Implied Volatility for a Put using Newton-Raphson."""
    d1 = (np.log(S / K) + (r + 0.5 * sigma ** 2) * T) / (sigma * np.sqrt(T))
    d2 = (np.log(S / K) + (r - 0.5 * sigma ** 2) * T) / (sigma * np.sqrt(T))
...
def get_implied_volatility(option_type, price, strike, time_to_exp, market_price, risk_free_rate=0.045):
    """
    Back-solves for IV. 
    Returns 0.0 if calculation fails (often due to deep ITM/OTM pricing anomalies).
    """
    ...
    try:
        for i in range(100):
            ...
            if abs(diff - sigma) < 1e-5:
                break
        return abs(sigma) # Return absolute value just in case
    except:
        return 0.0
...
def get_stock_data(ticker):
    ...
    # Since we can't get historical option prices for free, we use HV as the proxy for Rank.
    ...
    try:
        ...
        # Handle if calendar returns a list of dates or just one
        if isinstance(earnings_date, (list, tuple)):
             # Filter for future
             future = [d for d in earnings_date if d > datetime.now().date()]
             earnings_date = future[0] if future else None
        elif earnings_date < datetime.now().date():
             earnings_date = None # Passed
    except:
        earnings_date = None

    return {
        "price": current_price,
        "rank": rank, # This is HV Rank
...
def find_credit_spread(stock_obj, current_price, target_dte=30, width=5.0):
    ...
    if days_to_exp < 7: return None # Too close
    ...
    # 3. Find Short Leg (~30 Delta Proxy)
    # Without live greeks, we estimate 30 delta is roughly 4-5% OTM for average vol stocks.
    # We will grab the strike closest to Price * 0.96
    ...
    # 5. Calculate Exact IV for the Short Leg
...
def plot_cone_matplotlib(hist_data, current_price, ticker, iv):
    """
    Plots Price History + Cone of Expected Move using Matplotlib.
    Uses the Calculated Implied Volatility (IV) for the projection.
    """
    
    # Setup Data
    prices = hist_data['Close'].tail(90) # Last 90 days
    dates = prices.index
    
    # Calculate Cone
    days_proj = 30
    # Expected Move = Price * IV * sqrt(days/365)
    # We create a range of days 1-30
    proj_days = np.arange(1, days_proj + 1)
    
    # Calculate upper/lower bounds based on 1 Std Dev
    vol_move = current_price * (iv / 100) * np.sqrt(proj_days / 365)
    ...
    # Fill Cone
    ax.fill_between(future_dates, lower_cone, upper_cone, color='#00FFAA', alpha=0.15, label='Expected Move (1σ)')
    
    # Styling
    ax.set_title(f"{ticker} | Implied Vol: {iv:.1f}%", color='white', fontsize=10)
...
if st.button("🔎 Scan Liquid Tickers", type="primary"):
    ...
    for i, ticker in enumerate(LIQUID_TICKERS):
        ...
        # 3. Find Spread & Calc IV
        spread = find_credit_spread(data['obj'], data['price'])
