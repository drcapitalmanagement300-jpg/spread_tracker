# app.py (patched with IV30 rank for whole chain)
...
# ------------------- IV30 Rank for entire chain -------------------
@st.cache_data(ttl=60)
def fetch_chain_iv30_rank(ticker: str, expiration: date):
    """Fetch IV30 rank for the entire option chain (approx using last 30 days historical IV)."""
    try:
        ticker_obj = yf.Ticker(ticker)
        # Use at-the-money put IV as "current IV" approximation
        hist = ticker_obj.history(period="1mo")
        if hist.empty:
            return None
        current_price = hist['Close'].iloc[-1]
        # Fetch nearest expiration puts
        _, puts = get_option_chain(ticker, expiration.isoformat())
        if puts is None or puts.empty:
            return None
        atm_strike = min(puts['strike'], key=lambda x: abs(x - current_price))
        atm_iv = puts.loc[puts['strike']==atm_strike, 'impliedVolatility'].values[0]*100
        # Approximate 30-day min/max IV using last 30 days historical vol
        ivs = []
        for exp in ticker_obj.options:
            _, chain_puts = get_option_chain(ticker, exp)
            if chain_puts is not None and not chain_puts.empty:
                ivs.extend(chain_puts['impliedVolatility'].dropna().values*100)
        if not ivs:
            return None
...
def evaluate_rules(trade, derived, current_price, delta, current_iv, short_option_price, long_option_price):
    ...
    if derived["dte"] <= 7:
        rule_violations["other_rules"] = True
    entry_iv_rank = trade.get("entry_iv_rank")
    if entry_iv_rank and current_iv and current_iv > entry_iv_rank:
...
with st.form("add_trade", clear_on_submit=True):
    ...
    # Fetch IV30 rank dynamically when ticker is entered
    entry_iv_rank = None
    if ticker:
        entry_iv_rank = fetch_chain_iv30_rank(ticker, expiration)
        st.markdown(f"**Entry IV30 Rank:** {entry_iv_rank if entry_iv_rank is not None else 'N/A'}%")
    ...
    if submitted:
        if not ticker:
            ...
            if saved_to_drive:
                st.success(f"Added {ticker} — saved to Drive. Entry IV30 Rank: {entry_iv_rank if entry_iv_rank else 'N/A'}%")
            else:
...
if not st.session_state.trades:
    ...
    for i, t in enumerate(st.session_state.trades):
        ...
        delta, _, short_option_price = get_short_leg_data(t)
        ...
        # fetch current IV30 rank live
        current_iv_rank = fetch_chain_iv30_rank(t['ticker'], t['expiration'])

        rule_violations, abs_delta, spread_value_percent = evaluate_rules(
            t, derived, current_price, delta, current_iv_rank, short_option_price, long_option_price
        )
        ...
        if rule_violations["other_rules"]:
            ...
            status_icon = "⚠️"
            status_text = "Current IV30 Rank exceeds entry."
        else:
            status_icon = "✅"
            status_text = "All rules satisfied."
...
Entry IV30 Rank: {t.get("entry_iv_rank") if t.get("entry_iv_rank") else "N/A"}%  <br>
Current IV30 Rank: {current_iv_rank if current_iv_rank else "N/A"}%
</div>
""", unsafe_allow_html=True)
        ...
        with card_cols[1]:
            ...
            if current_iv_rank is None or t.get("entry_iv_rank") is None:
                iv_color = "black"
            elif current_iv_rank == t.get("entry_iv_rank"):
                iv_color = "yellow"
            elif current_iv_rank > t.get("entry_iv_rank"):
                iv_color = "red"
            else:
...
Entry IV30 Rank: {t.get("entry_iv_rank") if t.get("entry_iv_rank") else "N/A"}% | Current IV30 Rank: <span style='color:{iv_color}'>{current_iv_rank if current_iv_rank else "N/A"}%</span>
""", unsafe_allow_html=True)
...
st.markdown("---")
st.caption("Spread value uses actual option prices — alerts accurate, delta BSM-based, Entry/Current IV30 Rank shown.")
